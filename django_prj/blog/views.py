...
#     # blog 폴더 밑에 있는 index.html을 호출해줘
...
#       get : 특정 조건을 만족하는 단일 개체를 반환
#        fillter : 주어진 모든 조건을 만족하는 모든 객체를 반환
...
class PostCreate(LoginRequiredMixin, CreateView):
    model = Post  # Post의 레코드를 사용할 수 있게하는 기능
    ...
    def form_valid(self, form):
        ...
        if current_user.is_authenticated and (current_user.is_staff or current_user.is_superuser):  # 로그인한 사용자 여부 체크
            ...
            if tags_str:            # tags가 있는 경우에만 처리
                ...
                tags_list = tags_str.split(';')         # list 요소로 반환
...
def new_comment(request, pk):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST' :
            # 사용자롷부터 제출된 데이터를 이용하여  CommentForm의 인스턴스를 생성
            comment_form = CommentForm(request.POST)

            if comment_form.is_valid():         # is_vaild : 폼 처리시 유효성 검사
...
class PostSearch(PostList):
    paginate_by = None      # Postlist에는 정의되어 있지만, PostSearch에서는 정의하지 ㅇ낳는다.
