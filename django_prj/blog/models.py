...
class Category(models.Model):
    ...
    # SlugField : URL 에서 활용
    # allow_unicode=True : 모든 유니코드 문자를 지원
...
class Post(models.Model):
    ...
    # 해당 폴더를 만들어서 년,월,일 로 내려가서 저장하도록 설정 / black=TRUE : 필수기능아님
    ...
    # ManyToMany... 다대다 관계 성립함
    tags = models.ManyToManyField(Tag, blank=True)

    # 파이썬 내장 메서드로, 클래스의 인스턴스를 문자열로 표현하는데 사용
    def __str__(self):
        return f'[{self.pk}] {self.title} :: {self.author}'

    def get_absolute_url(self):     #get_absilute_url, 사용자 정의 함수
